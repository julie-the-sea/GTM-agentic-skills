# ABM 1:Few Journey Design

A small, dependency-free Python script that generates a system prompt for
a **1:Few ABM (Account-Based Marketing) journey design assistant** — an
AI assistant that turns a cluster of ~15 target accounts, a buying
group, and your existing content toolkit into a 3–6 month campaign
journey draft.

It's agent-agnostic by design: `abm_journey_design.py` doesn't call any
AI API and isn't built around any single AI product. It just renders
text. Paste the output into whatever your team already uses to run a
persistent assistant — a custom agent, a project/assistant feature in a
chat product, or a system prompt passed directly via any provider's API.

This started life as a single hardcoded prompt built for one company's
product. This version pulls every company-specific detail (name,
product areas, jargon, systems of record) into config, so any team can
generate their own version without editing prose by hand.
...
```bash
python3 abm_journey_design.py --company-name "Acme Corp" -o prompt.txt
```

To drive it from a config file instead of flags:

```bash
python3 abm_journey_design.py --write-sample-config config.json
# edit config.json to match your company
python3 abm_journey_design.py --config config.json -o prompt.txt
```
...
1. **The prompt** (`build_journey_prompt`) — the actual instructions the
   AI assistant runs on every session: its role, the discovery questions
   it must ask before drafting anything, the research it should do, and
   the exact structure of the deliverable (journey logic, a
   month-by-month journey table, a content gap analysis, and a closing
   instruction).
...
| `role_title` | `--role-title` | `1:Few Journey Designer` | Appended after the company name to form the assistant's role. |
...
aren't a config toggle, and the prompt itself instructs the AI assistant
not to let later chat messages override them:
...
| Sanity-check every AI-generated suggestion against your own source reports before using it. | Treat the AI assistant's output as a system of record — it's a drafting aid, not ground truth. |
...
They exist so the AI assistant never sees individually identifiable
customer data, financial/health/HR/legal content, or anything that
should live only in a system of record.

## Using the output with any AI tool

The generated text is a plain system prompt / custom-instructions block
with no vendor-specific syntax. For any AI tool or agent framework:

1. Generate the prompt (`python3 abm_journey_design.py ... -o prompt.txt`).
2. Create a new persistent assistant wherever your team runs one — a
   custom agent, a project or assistant feature in your chat product of
   choice, or the `system` parameter of a direct API call — and paste
   `prompt.txt` in as its instructions.
...
syntax baked in, it works the same way regardless of which AI product
or agent framework you paste it into — there's nothing in this repo to
swap out when your team changes AI vendors.

## Extending it

- Want a different deliverable structure? Edit the f-string in
  `build_journey_prompt()` in `abm_journey_design.py` — it's plain text,
  not a templating engine, so it's easy to read and easy to change.
- Want more config-driven fields (e.g., a fixed list of allowed
  channels)? Add a field to `JourneyConfig`, wire it into the f-string,
  and add a matching CLI flag in `_build_arg_parser()`.
...
- `abm_journey_design.py` — the prompt-builder script (CLI + importable
