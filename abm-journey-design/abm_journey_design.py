#!/usr/bin/env python3
"""
abm_journey_design.py

A model-agnostic prompt builder for a 1:Few ABM (Account-Based Marketing)
journey-design assistant.

This does NOT call any AI API and is not tied to any single AI product.
It renders a complete system-prompt / instruction set — plain text you
paste into whatever AI tool or agent framework your team already uses —
to turn that tool into a reusable "journey designer" for your own
product and buying groups. That could be a persistent-assistant feature
in a chat product, a custom agent built on an SDK, or a system prompt
passed directly via any provider's API.
...
Quickstart
----------
    python3 abm_journey_design.py --company-name "Acme Corp" \\
    ...
    python3 abm_journey_design.py --write-sample-config my_config.json
    python3 abm_journey_design.py --config my_config.json --output journey_prompt.txt
...
class JourneyConfig:
    ...
    role_title: str = "1:Few Journey Designer"
    ...
    # What counts as "raw source data" the assistant must never receive.
    ...
    @classmethod
    def from_json(cls, path: str) -> "JourneyConfig":
...
def build_journey_prompt(cfg: JourneyConfig) -> str:
...
You are the "{cfg.company_name} {cfg.role_title}." Your purpose is to architect a {dmin}-{dmax} month scalable marketing journey map for clusters of ~{cfg.buying_group_size} high-value accounts (e.g., a Buying Group of {roles}) to drive Account Engagement, Champion Development, and Pipeline Progression. You focus on "personalization at scale," blending a user's existing Content Toolkit with AI-driven Gap Analysis to ensure messaging depth evolves as an account matures.
...
def build_data_inputs_checklist(cfg: JourneyConfig) -> str:
...
def _build_arg_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        description="Render a model-agnostic 1:Few ABM journey-design prompt.",
    ...
    p.add_argument("--role-title", help="The assistant's role/title, appended after the company name.")
...
def _apply_cli_overrides(cfg: JourneyConfig, args: argparse.Namespace) -> JourneyConfig:
    overrides = {}
    simple_fields = [
        "company_name", "role_title", "buying_group_size",
...
def main(argv: List[str] = None) -> int:
    ...
    if args.write_sample_config:
        JourneyConfig().to_json(args.write_sample_config)
        print(f"Sample config written to {args.write_sample_config}", file=sys.stderr)
        return 0

    cfg = JourneyConfig.from_json(args.config) if args.config else JourneyConfig()
    cfg = _apply_cli_overrides(cfg, args)

    output = build_journey_prompt(cfg)
